...
class QdrantSettings(BaseSettings):
    ...
    vector_size: int = Field(default=1024)  # BAAI/bge-large-en-v1.5
...
class RedisSettings(BaseSettings):
    ...
    ttl_search: int = Field(default=3600)       # 1 hour for search results
    ttl_embedding: int = Field(default=86400)   # 24 hours for embeddings
    ttl_session: int = Field(default=86400 * 7) # 7 days for sessions
...
class LLMSettings(BaseSettings):
    ...
    provider: str = Field(default="groq")  # groq | ollama | openai
...
class RetrievalSettings(BaseSettings):
    ...
    hybrid_top_k: int = Field(default=40)
...
class Settings:
    """Aggregated settings object. Use get_settings() to access."""
